...
/* ================================
   Veloura Floating Categories Active Sync
   يفعّل عنصر التصنيفات في القائمة العائمة عند فتح القائمة من أي زر
================================ */
...
(function () {
  ...
  function getFloatingItems() {
    return Array.from(document.querySelectorAll('.veloura-mobile-floating-menu__item'));
  }

  function isCategoriesFloatingItem(item) {
    if (!item) return false;

    var href = item.getAttribute('href') || '';
    var action = item.getAttribute('data-action') || item.getAttribute('data-veloura-action') || '';
    ...
    return (
      ...
      action === 'categories' ||
      action === 'category' ||
      /التصنيفات|تصنيفات|categories|category/i.test(text)
    );
  }

  function getCategoriesFloatingItem() {
    return getFloatingItems().find(isCategoriesFloatingItem);
  }

  function isMobileMenuOpen() {
    return !!document.querySelector('.mm-ocd.mm-ocd--open #mobile-menu');
  }

  function setCategoriesActive(active) {
    var items = getFloatingItems();
    var categoriesItem = getCategoriesFloatingItem();
    ...
    if (active) {
      ...
      categoriesItem.classList.add('is-active');
      document.body.classList.add('veloura-floating-categories-active');
      return;
    }

    categoriesItem.classList.remove('is-active');
    document.body.classList.remove('veloura-floating-categories-active');
  }

  function syncFloatingCategoriesActive() {
    setCategoriesActive(isMobileMenuOpen());
  }

  document.addEventListener('DOMContentLoaded', function () {
    syncFloatingCategoriesActive();
    ...
    setTimeout(syncFloatingCategoriesActive, 300);
    setTimeout(syncFloatingCategoriesActive, 900);
  });

  document.addEventListener('click', function (event) {
    if (
      event.target.closest("a[href='#mobile-menu']") ||
      event.target.closest('.veloura-menu-trigger-mobile') ||
      event.target.closest('.veloura-menu-trigger-desktop')
    ) {
      setTimeout(syncFloatingCategoriesActive, 80);
      setTimeout(syncFloatingCategoriesActive, 250);
      setTimeout(syncFloatingCategoriesActive, 700);
    }

    if (
      event.target.closest('.close-mobile-menu') ||
      event.target.closest('.mm-ocd__backdrop')
    ) {
      setTimeout(syncFloatingCategoriesActive, 80);
      setTimeout(syncFloatingCategoriesActive, 250);
    }
